...
public class Chairman extends Member{
  static int competiviveCounter = 1;
  static int casualCounter = 1;

  MemberList competiviveList = new MemberList();
  ...
  public void registerCasual(){
    ...
    competiviveList.getCompetiviveList().add(member);
    System.out.println(competiviveList.getCompetiviveList());
  }
  public void registerCompetivive(){
    ...
    if (getAgeGroup().equalsIgnoreCase("Junior") && getStatus().equalsIgnoreCase("Aktiv")){
      ...
      setBalance(-1600);
    } else if (getAgeGroup().equalsIgnoreCase("Pensioner") && getStatus().equalsIgnoreCase("Aktiv")){
      setBalance(-1200);
    } else
      setBalance(-500);
    ...
    Member member = new Member(getName(),getAge(),getStatus(),getAgeGroup(),getBalance());
    competiviveList.getCompetiviveList().add(member);
    System.out.println(competiviveList.getCompetiviveList()); //Denne er kun for at tjekke om Medlemmen blev tilføjet i
    //ArrayListen
      }
    }
